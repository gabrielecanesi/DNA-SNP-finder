...
Find::Find(const std::shared_ptr<std::string>& reference, const std::shared_ptr<std::string>& r, size_t k, size_t firstK,
           double bloomFilterThreshold, double firstThreshold) :
           CliCommand(reference, r, k, firstK,
                      bloomFilterThreshold, firstThreshold) {}


void Find::run() {
    auto begin = std::chrono::steady_clock::now();
    size_t snpPosition = algorithms::findSNPPosition(reference, r, k, firstK, bloomFilterThreshold, firstThreshold);
