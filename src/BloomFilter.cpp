...
BloomFilter::BloomFilter(size_t size) : size(size), M_numberOfInserts(0) {
    if (size % sizeof(bool) > 0) {
        size += sizeof(bool);
    }

    array = new bool[size / sizeof(bool)];
    std::fill(array, array + size / sizeof(bool), 0);
}

size_t BloomFilter::hash(uint64_t element, size_t number) const {
    auto h1 = (hashValues[0] * element) % (size * sizeof(bool));
    auto h2 = number * (hashValues[0] * element) % (size * sizeof(bool));
    return h1 + h2;
}
